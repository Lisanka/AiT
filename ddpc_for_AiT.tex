...
где $y_0 = \{y_0(t)\}_{t=\tau}^{T-1}$ --- выходной сигнал системы $G$, соответствующий управлению $u$ и тривиальному начальному условию $x(\tau) = 0$; {\color{olive} Это место по-прежнему не нравится, потому что было договорено, что $\{u_\tau^p, y_\tau^p\}$  фиксированная нам неизвестная траектория, реализовавшаяся в конкретном процессе} $\hat y = \{\hat y(t)\}_{t=\tau}^{T-1}$ --- выходной сигнал неуправляемой системы $G$ для некоторого начального условия $x(\tau)$, согласующегося с прошлой траекторией $\{u_\tau^p, y_\tau^p\}$.
...
3) подать на вход системы управление $u^p(\tau) = U_{\tau}^f(\tau)\alpha_0^*(\tau, u_\tau^p, \tilde{y}_\tau^p)$.
...
$u^*(\tau|\tau) = u^*(\tau|\tau, u^p_\tau, \tilde y^p_\tau)$, а также для момента $\tau+1$ и позиции $\{u^p_\tau, u^*(\tau|\tau), \tilde y^p_\tau, \tilde y^p(\tau)\}$, которая реализуется при подаче управления $u^*(\tau|\tau)$ в момент $\tau$.
...
\section{Примеры}
\subsection{Двумерная система}
Применим предложенный алгоритм для управления cледующей системой:
...
\end{align}
Требуется минимизировать величину $\sum_{t=0}^{T-1}\norm{u(t)}^2$ при гарантированном соблюдении ограничений на значения выходных сигналов в последних $15$ временных точках:   $ |y(t)| \leq 0.3,$ $t = T-15, \ldots, T-1$. Доступны управления, для которых $|u(t)| \leq 0.8$,  $t = 0, \ldots, T-1$. Горизонт планирования $T=135$, абсолютное значение ошибки ограничено величиной $\varepsilon = 0.02$. 
...
На рис. \ref{example1-u} изображены реализации $u^p(\tau)$, $\tau = 0, \ldots, T-1$, оптимальной обратной связи в рассматриваемом процессе.  Выходные сигналы (см. рис. \ref{example1-y}) удовлетворяют ограничениям на промежутке от $\tau = 120$ до $\tau=134$. Априорная оценка значения критерия качества, полученная в $\tau = 2$ составила $ 46.1799$. В процессе управления значение критерия качества улучшилось до $41.6098$. Наиболее значительное изменение происходит при $\tau = 2, \ldots, 28$.

\begin{figure}[h!]
\centering
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=\linewidth]{2du-final.eps}
\caption{}
\label{example1-u}
\end{subfigure}\hfil
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=\linewidth]{2dy-final.eps}
\caption{}
\label{example1-y}
\end{subfigure}
...
\begin{figure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=\linewidth]{2dx-final.eps}
\caption{}
\end{subfigure}\hfil
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=\linewidth]{2dcost-final.eps}
\caption{}
\end{subfigure}
\caption{Фазовая траектория (c), изменение критерия качества (d)}
\end{figure}


\subsection{Четырехмерная система}
Поставим задачу оптимального управления для системы, которая была рассмотрена в \cite{BerberichMPC}:
...
Рис. \ref{fig:example2} иллюстрирует результаты применения предложенной схемы управления. Априорная оценка значения критерия качества, составлявшая $32.183$,  была улучшена до $21.857$. Начиная с момента $\tau = 29$ изменения не столь существенны. {\color{red} или 29 здесь?}{\color{blue} в тау=29 уже почти такое же, что и в тау=28 значение}
...
\caption{}
\end{subfigure}
\caption{Реализованная оптимальная обратная связь (a)--(b), соответствующие выходные сигналы (c), изменение критерия качества (d)}
\label{fig:example2}
\end{figure}
...
Таким образом, решение задачи (\ref{oc-at-tau}) может быть получено в результате решения $\color{blue} q(T-\tau)$ %$ q(T-\tau+1)$ 
