...
Управление $u^p(\tau)$, $\tau = 0, \ldots, T-1$, построенное по правилу (\ref{fb0}) представляет собой \cite{Dmitruk2004} реализацию в конкретном процессе управления оптимальной обратной связи по неточным измерениям.
...
{\color{blue} я везде с этого момента заменила просто $\alpha$ на $\alpha(\tau)$: пусть будет одинаково}
...
U_{\tau}^f \\
Y_{\tau}^f
\end{pmatrix} \alpha{\color{blue}(\tau)} = \begin{pmatrix}
u_{\tau}^p \\
y_{\tau}^p \\
u \\
y
...
где $y_0 = \{y_0(t)\}_{t=\tau}^{T-1}$ --- выходной сигнал системы $G$, соответствующий управлению $u$ и тривиальному начальному условию $x(\tau) = 0$; {\color{blue} вот так про $\hat y$?}$\hat y = \{\hat y(t)\}_{t=\tau}^{T-1}$ --- выходной сигнал неуправляемой системы $G$ для некоторого начального условия $x(\tau)$, согласующегося с прошлой траекторией $\{u_\tau^p, y_\tau^p\}$.
...
Далее $\{0,\hat y\}$ --- траектория системы $G$ c начальным условием  $x(\tau)$ из множества состояний, в которые система могла прийти, точно следуя траектории $\{u^p, y_\tau^p\}$. Тогда $\{u_{\tau}^p, 0, y_{\tau}^p, \hat{y}\}$ --- траектория длины $T$ системы $G$.
...
1. Найти некоторые решения $\hat{\alpha}{\color{blue}(\tau)}, \alpha_0{\color{blue}(\tau)}$ алгебраичеких уравнений
\begin{equation}\label{cond-lemma}
\begin{pmatrix}
U_\tau^p \\
Y_\tau^p \\
U_\tau^f \\
\end{pmatrix} \hat{\alpha}{\color{blue}(\tau)} = \begin{pmatrix}
u_\tau^p \\
y_\tau^p \\
0 \\
\end{pmatrix}, \quad
...
Y_\tau^p \\
U_\tau^f \\
\end{pmatrix} \alpha_0{\color{blue}(\tau)} = \begin{pmatrix}
0 \\
0 \\
u \\
\end{pmatrix}.
\end{equation}
2. Вычислить $\hat{y} = Y_\tau^f\hat{\alpha}{\color{blue}(\tau)}$, $y_0 = Y_\tau^f\alpha_0{\color{blue}(\tau)}$.
\end{lemma}
...
$$  
     ...
     $$
Получим $\hat y(t) = Y^d(t)\hat \alpha{\color{blue}(\tau)}$, $y_0(t) = Y^d(t) \alpha_0{\color{blue}(\tau)}$.
\end{remark}
...
Множество ${\cal A}(\tau, u_\tau^p, \tilde y^p_\tau)$ --- аналог множества $X(\tau, u_\tau^p, \tilde y^p_\tau)$ состояний $x(\tau)$, согласующихся с измеренной траекторией $\{u_\tau^p, \tilde y^p_\tau\}$.
...
\end{equation}
где каждое $\chi(t|\tau) = (\chi_i(t|\tau), i = 1,\ldots, q)$ соответствует наихудшей реализации выходного сигнала $\hat y(t)$ в позиции $\{u_\tau^p, \tilde y^p_\tau\}$, а именно, каждый его элемент $\chi_i(t|\tau)$ является решением задачи
...
Выполнение  для $y_0(t)$ суженного ограничения (\ref{tightened constraint}) очевидно влечет удовлетворение исходного ограничения (\ref{constraints}) на $y(t)$ для любой возможной реализации $\hat{y}(t)$,  согласующейся с позицией $\{u^p_\tau, \tilde y^p_\tau\}$.
...
{\color{olive}Таким образом, задача оптимального управления, решаемая в каждой временной точке $n \leq \tau \leq T-1$, свелась к ряду задач линейного программирования и одной выпуклой задаче квадратичного программирования.}


\subsection{Основная теорема}


Дадим теоретическое обоснование реализуемости предложенного алгоритма управления. В частности, покажем, что при условии существования решений задач (\ref{estimation-lp}) и (\ref{control}) в момент времени $\tau = n$, решения этих задач существуют и для всех $\tau = n+1, \ldots, T-1$.
%следующая теорема доказывает рекуррентную разрешимость задач  (\ref{estimation-lp}) и (\ref{control}), решаемых в каждый момент времени $\tau = n, \ldots, T-1$.

С целью сокращения записи далее  опустим явное упоминание пары $\{u^p_\tau, \tilde y^p_\tau\}$ 
%{\color{blue} тут не только после вертикальной черты, но и после запятой} %после вертикальной черты 
в решениях задачи (\ref{control-qp}) для момента $\tau$, т.е.
$\alpha_0^*(\tau) = \alpha_0^*(\tau, u^p_\tau, \tilde y^p_\tau)$,
$u^*(\tau|\tau) = u^*(\tau|\tau, u^p_\tau, \tilde y^p_\tau)$. То же касается момента $\tau+1$ и позиции $\{u^p_\tau, u^*(\tau|\tau), \tilde y^p_\tau, \tilde y^p(\tau)\}$, которая реализуется при подаче управления $u^*(\tau|\tau)$ в момент $\tau$. {\color{red} Может, нам вообще по всему тексту опустить эти пары??}{\color{blue}Мне кажется, не стоит: так читается приятнее текст в Приложении и в разделе 2}

\begin{theorem}\label{main-theorem}
Пусть в момент $\tau$ для позиции $\{u^p_\tau, \tilde y^p_\tau\}$ каждая из задач (\ref{estimation-lp}) имеет решение. Далее, пусть имеет решение соответствующая задача (\ref{control-qp}) с параметрами $\chi(t|\tau)$, поставленными решениями задач (\ref{estimation-lp}). Тогда в момент $\tau+1$ для позиции $\{u^p_\tau, u^*(\tau|\tau), \tilde y^p_\tau, \tilde y^p(\tau)\}$ эти задачи (\ref{estimation-lp}), (\ref{control-qp})   также имеют решение. Более того, $$
...
\begin{gather}\label{estimation-lp-tau+1}
...
\begin{gather}\label{estimation-lp-tau} 
    \chi_i(t|\tau) = \max\limits_{\hat{\alpha}(\tau)} G_i(t)Y^d(t)\hat{\alpha}(\tau), \\
    \begin{pmatrix}
        U_{\tau}^p \\
        U^d(\tau)\\
...
{\color{blue} абзац про допустимость}
...
%Для построения требуемого $\hat\alpha(\tau+1)$ 
{\color{blue}
...
%Заметим, что  $\alpha_0^*(\tau)$ удовлетворяет
%\begin{equation}\label{alpha_0-tau}
%    \begin{pmatrix}
%    U_{\tau}^p \\
%    U^d(\tau)\\
%    Y_{\tau}^p \\
...
%    \end{pmatrix},
%    \end{equation}
%\begin{equation}\label{alpha_0-tau-y}
%    G(t)Y^d(t) \alpha_0^*(\tau) \leq g(t)-\chi(t|\tau), \ \
%    t = \tau+1, \ldots, T-1.
%\end{equation}
Обозначим 
\begin{equation}\label{uc}
    ...
    \end{equation}
Заметим, что $u^c = \{u^*(t|\tau)\}_{t = \tau{\color{blue}+1}}^{T-1}$, т.е. это "хвост"{} оптимального управления задачи (\ref{control-qp}). Согласно Лемме \ref{dd-separation} существует $\alpha^c$, такое что выполняется равенство
...
\begin{gather} 
    ...
    \begin{pmatrix}
        ...
        U^d(\tau)\\
        U_\tau^f \\
    \end{pmatrix} \Delta\alpha({\color{blue}\tau}+1) = 
    \begin{pmatrix}
        u_\tau^p \\
        0 \\
        0\\
    \end{pmatrix}, \label{u-delta-alpha}\\
    \tilde{y}_{\tau}^p-\varepsilon \mathbbm{1}  \leq
...
%{\color{red} Его элементом является $\hat\alpha^p(\tau+1)$, соответствующий истинной траектории $\{u^p_\tau, 0, y^p_\tau, \hat y^p(\tau|\tau)\}$ согласно Лемме \ref{dd-separation}:
...
%\end{equation*}
%
%Действительно, для $\hat\alpha^p(\tau+1)$ ограничения (\ref{u-delta-alpha}), (\ref{y-delta-alpha}) очевидно удовлетворены, а (\ref{estimation-new-constraint}) выполняется ввиду того, что по принципу разделимости $\hat y^p(\tau|\tau)=y^p(\tau)-  y_0^*(\tau|\tau)$.
%
%
...
%\chi(\tau+1) \leq G_{\tau+1}Y_{\tau+1}^f(\alpha_0^*(\tau)-\alpha^c) + \chi_{\tau+1}(\tau)
%\end{equation}
%}

Сравним задачи (\ref{estimation-delta-alpha}) -- (\ref{estimation-new-constraint}) для $\Delta \alpha(\tau + 1)$ и (\ref{estimation-lp-tau}) для $\hat \alpha (\tau)$. На $\Delta \alpha(\tau+1)$ наложено дополнительное ограничение (\ref{estimation-new-constraint}), поэтому $\Delta\chi_i(t|\tau+1) \le \chi_i(t|\tau)$ и справедлива оценка
\begin{equation}\label{chi-bound}
...
%\begin{gather}
%\min\limits_{\alpha_0(\tau)}\alpha_0(\tau)^T{U_{\tau}^f}^TU_{\tau}^f\alpha_0(\tau), \label{control-qp-tau}\\
%\begin{pmatrix}
%U_{\tau}^p \\
%Y_{\tau}^p \\
%\end{pmatrix} \alpha_0(\tau) = \begin{pmatrix}
...
\begin{align*}
    G(t) Y^d(t) \alpha^c + \chi({\color{blue}t|}\tau+1) & \leq G(t) Y^d(t) \alpha^c +  G(t)Y^d(t)(\alpha_0^*(\tau)-\alpha^c) + \chi(t|\tau) = \\
...
\begin{corollary}
Если задачи оптимального наблюдения (\ref{estimation-lp}) и управления (\ref{control-qp}) имеют решение в момент времени $\tau=n$, то они разрешимы и в каждый из последующих моментов $\tau = n+1, \ldots, T-1$. {\color{blue} тут слово "функция" два раза подряд шло, мб так переписать:} При этом критерий качества, значение которого определяется в момент $\tau$ как 
$$
    J(\tau) = \sum\limits_{t=0}^{\tau-1}\norm{u^p(t)}^2 + \sum\limits_{t=\tau}^{T-1}\norm{u^*(t|\tau)}^2
    $$
является невозрастающей функцией от $\tau$.

%При этом значение критерия качества как функция $\tau$
%$$
%    J(\tau) = \sum\limits_{t=0}^{\tau-1}\norm{u^p(t)}^2 + \sum\limits_{t=\tau}^{T-1}\norm{u^*(t|\tau)}^2
%    $$
...
минимизируя критерий качества $\sum_{t=0}^{T-1}\norm{u(t)}^2$ при гарантированном соблюдении ограничений на значения выходных сигналов в последних $20$ временных точках:   $ |y_i(t)| \leq 0.1,$ $i=1,2,$ $t = T-20, \ldots, T-1$. Доступны управления, для которых $|u_i(t)| \leq 0.8,$ $i=1,2,$ $t = n, \ldots, T-1$ 

Момент начала управления такой же, как и размерность системы $n = 4$. Отметим, что для данной системы управление можно начать раньше, в момент $t=2$, --- этого достаточно для неявного задания начального условия $x_0$, --- но будем предполагать, что известна лишь точная размерность системы. Горизонт планирования $T = 55.$

Как было отмечено в Замечании \ref{remark on input length}, длина априорной траектории $T^d$ должна быть не меньше $(n + T)(m + 1) - 1$, что в рассматриваемом случае, при $n = 4, m = 2$, означает $T^d \geq 3(4+T) - 1 = 176.$ Именно такой, наименьшей подходящей длины, была предварительно сгенерирована незашумленная траектория $\{u^d, y^d\}.$ Входные сигналы $u^d(t),$ $t=0,\ldots,T^d-1$ были сгенерированы как случайные векторы, равномерно распределенные в $[-0.8, 0.8]^2$.

После для начального состояния $x(0) = \begin{pmatrix}
4 & 0 & 1&-1
\end{pmatrix}^T$  была сгенерирована зашумленная траектория $\{u^p_4, \tilde{y}^p_4\}$, соответствующая движению системы до начала управления: входные сигналы $u^p(t),$ $t=0,\ldots,3$ были выбраны случайным образом из квадрата $[0, 0.8]^2$, а ошибки, добавляемые к "чистым"\ выходным сигналам $y^p(t),$ $t=0,\ldots,3$, --- из $[-\varepsilon, \varepsilon]^2,$ для $\varepsilon = 0.02.$

Ниже приведены управление и выход, полученные в результате применения предложенного метода. Оптимальное управление удовлетворяет ограничениям (штрих-пунктирные прямые на графике) на всем временном интервале и активно на некоторых его промежутках.
...
\caption{}
\end{subfigure}
\caption{Реализованное оптимальное управление (a)--(b), соответствующие выходные сигналы (c), изменение критерия качества (d)}
\end{figure}

Выходная траектория также удовлетворяет ограничениям (заключена в области между двумя штрих-пунктирными линиями на графике) на промежутке от $\tau = 35$ до $\tau=54$. 

Значение критерия качества составляет $32.183$ на первой итерации, $21.857$ --- на последней. Наиболее значительное изменение происходит на первых $24$ итерациях.
...
Задача решена в предположении, что траектория, измеряемая в процессе управления, неточна, но априорная траектория известна точно. {\color{blue} следующее предложение просто убрать? мы же ничего не предлагали в этой работе.} Было предложено дальнейшее направление исследования: случай, когда априорная траектория также зашумлена.
...
$$
    \hat{x}(t{\color{blue}+1}) = A\hat{x}(t), \quad \hat{x}(\tau) \in X(\tau, u_\tau^p, \tilde{y}_\tau^p),
    $$
$$
...
{\color{blue} затрудняюсь предположить, что про себя писать:) "Студент, Белорусский государственный университет" или вообще пустое место оставить? И не уверена, что корректно про Вас написала.}

\AdditionalInformation{Дмитрук Н.М.}{Белорусский государственный университет, заведующий кафедрой методов оптимального управления ФПМИ, Минск}{dmitrukn@bsu.by}

\AdditionalInformation{Манжулина Е.А.}{Белорусский государственный университет, студент, Минск}{fpm.manzhuli@bsu.by}
